...
class SquareMaze
{
  public:
     ...
     */
    cs225::PNG* drawMaze();
    void initializeSearch(std::vector<int>& dist, std::vector<int>& prev, std::queue<int>& q);
    void performBFS(std::vector<int>& dist, std::vector<int>& prev, std::queue<int>& q);
    int findFurthestBottomCell(const std::vector<int>& dist);
    std::vector<int> retracePath(const std::vector<int>& prev, int start, int end);
    std::vector<int> solveMazesub();
    int end = 0;
    ...
    int width;
    int height;
    std::vector<std::vector<bool>> rightWalls, downWalls;
    const std::vector<int> xDir = {1, 0, -1, 0};
    const std::vector<int> yDir = {0, 1, 0, -1};
