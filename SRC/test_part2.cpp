...
void sprintMaze(std::ostream &s, SquareMaze const &maze, int width,
  ...
  auto canTravel = [&](int x, int y, int dir) -> bool {
    if (x == 0 && y == -1 && dir == 1)
      return true;
    if ((y < 0 || y >= height) && dir == 0)
      return true;
    if ((x < 0 || x >= width) && dir == 1)
      return true;
  ...
  auto index = [&](int x, int y) -> char {
    // clang-format off
    return !canTravel(x    , y - 1, 1) << 3
         | !canTravel(x - 1, y    , 0) << 2
         | !canTravel(x - 1, y - 1, 1) << 1
         | !canTravel(x - 1, y - 1, 0) << 0;
    // clang-format on
  };
...
TEST_CASE("my case", "[weight=200000][part2][timeout=30000]")
{
    PNG solnImage = READ_SOLUTION_PNG("testDrawSolutionLarge", 200, 200);
    MazeReader soln(solnImage);
    SquareMaze maze;
    ...
    PNG * actualmaze = maze.drawMaze();
        actualmaze->writeToFile("testSelf"+ string("-actual.png"));

    maze.solveMaze();
    PNG * actualOutput = maze.drawMazeWithSolution();
    actualOutput->writeToFile("testDrawSolutionSelf"+ string("-actual.png"));
